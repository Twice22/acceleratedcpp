...
public:
	...
	*/
	...
	// Answer to Q12.14 add constructor
	Vec(std::string& s);
...
template <class T>
template <class In>
void Vec<T>::insert(iterator pos, In b, In e) {
	// allocate new space and copy existing elements to the new space
	size_type new_size = size() + (e - b);
	iterator new_data = alloc.allocate(new_size);

	// Add data before inserted datas
	iterator tmp = std::uninitialized_copy(data, pos, new_data);
	iterator tmp2 = std::uninitialized_copy(b, e, tmp);
	iterator new_avail = std::uninitialized_copy(pos, end(), tmp2);
...
void Vec<T>::assign(In b, In e) {
	uncreate(); // don't forget to uncreate
	create(b, e);
}


// add << operator from conveniency
// Add << operator for conveniency
template <class T>
std::ostream& operator<<(std::ostream& os, const Vec<T>& v) {
	for (Vec<T>::const_iterator it = v.begin(); it != v.end(); ++it) {
		os << *it;
	}
	return os;
}
...
template <class T>
Vec<T>::Vec(std::string& s) {
	const char* temp = s.c_str();
	create(temp, temp + std::strlen(temp));
}
